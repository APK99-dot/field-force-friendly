export interface HierarchicalItem {
  name: string;
  label: string;
}

export interface HierarchicalModule {
  module: string;
  label: string;
  fields: HierarchicalItem[];
  actions: HierarchicalItem[];
  widgets: HierarchicalItem[];
}

export const HIERARCHICAL_MODULES: HierarchicalModule[] = [
  {
    module: "module_admin_panel",
    label: "Admin Panel",
    fields: [
      { name: "field_admin_dashboard", label: "Dashboard" },
      { name: "field_admin_user_management", label: "User Management" },
      { name: "field_admin_attendance_mgmt", label: "Attendance Management" },
      { name: "field_admin_expense_mgmt", label: "Expense Management" },
      { name: "field_admin_gps_tracking", label: "GPS Tracking" },
      { name: "field_admin_security_access", label: "Security & Access" },
      { name: "field_admin_company_profile", label: "Company Profile" },
    ],
    actions: [
      { name: "action_admin_create_user", label: "Create User" },
      { name: "action_admin_edit_user", label: "Edit User" },
      { name: "action_admin_delete_user", label: "Delete User" },
      { name: "action_admin_manage_holidays", label: "Manage Holidays" },
      { name: "action_admin_manage_leave_types", label: "Manage Leave Types" },
      { name: "action_admin_approve_expense", label: "Approve Expense" },
      { name: "action_admin_manage_profiles", label: "Manage Security Profiles" },
    ],
    widgets: [
      { name: "widget_admin_user_list", label: "User List Widget" },
      { name: "widget_admin_attendance_overview", label: "Attendance Overview Widget" },
      { name: "widget_admin_expense_summary", label: "Expense Summary Widget" },
    ],
  },
  {
    module: "module_attendance",
    label: "Attendance",
    fields: [
      { name: "field_attendance_check_in_time", label: "Check-in Time" },
      { name: "field_attendance_check_out_time", label: "Check-out Time" },
      { name: "field_attendance_total_hours", label: "Total Hours" },
      { name: "field_attendance_location", label: "Location" },
      { name: "field_attendance_photo", label: "Photo" },
      { name: "field_attendance_face_verification", label: "Face Verification" },
    ],
    actions: [
      { name: "action_attendance_check_in", label: "Check In" },
      { name: "action_attendance_check_out", label: "Check Out" },
      { name: "action_attendance_regularize", label: "Request Regularization" },
    ],
    widgets: [
      { name: "widget_attendance_calendar", label: "Attendance Calendar" },
      { name: "widget_attendance_records_table", label: "Attendance Records Table" },
      { name: "widget_attendance_summary_cards", label: "Summary Cards" },
    ],
  },
  {
    module: "module_activities",
    label: "Activities",
    fields: [
      { name: "field_activity_name", label: "Activity Name" },
      { name: "field_activity_type", label: "Activity Type" },
      { name: "field_activity_duration", label: "Duration" },
      { name: "field_activity_location", label: "Location" },
      { name: "field_activity_attachments", label: "Attachments" },
    ],
    actions: [
      { name: "action_activity_log", label: "Log Activity" },
      { name: "action_activity_manage_types", label: "Manage Activity Types" },
    ],
    widgets: [
      { name: "widget_activity_list", label: "Activity List" },
      { name: "widget_activity_summary", label: "Activity Summary" },
    ],
  },
  {
    module: "module_visits",
    label: "Visits",
    fields: [
      { name: "field_visit_retailer", label: "Retailer" },
      { name: "field_visit_purpose", label: "Purpose" },
      { name: "field_visit_location", label: "Location" },
      { name: "field_visit_notes", label: "Notes" },
      { name: "field_visit_photos", label: "Photos" },
    ],
    actions: [
      { name: "action_visit_plan", label: "Plan Visit" },
      { name: "action_visit_start", label: "Start Visit" },
      { name: "action_visit_complete", label: "Complete Visit" },
    ],
    widgets: [
      { name: "widget_visit_list", label: "Visit List" },
      { name: "widget_visit_summary", label: "Visit Summary" },
    ],
  },
  {
    module: "module_beats",
    label: "Beats",
    fields: [
      { name: "field_beat_name", label: "Beat Name" },
      { name: "field_beat_plan_date", label: "Plan Date" },
      { name: "field_beat_data", label: "Beat Data" },
    ],
    actions: [
      { name: "action_beat_create_plan", label: "Create Beat Plan" },
      { name: "action_beat_edit_plan", label: "Edit Beat Plan" },
    ],
    widgets: [
      { name: "widget_beat_plan_calendar", label: "Beat Plan Calendar" },
      { name: "widget_beat_list", label: "Beat List" },
    ],
  },
  {
    module: "module_expenses",
    label: "Expenses",
    fields: [
      { name: "field_expense_amount", label: "Amount" },
      { name: "field_expense_category", label: "Category" },
      { name: "field_expense_bill", label: "Bill/Receipt" },
      { name: "field_expense_date", label: "Expense Date" },
      { name: "field_expense_status", label: "Status" },
    ],
    actions: [
      { name: "action_expense_submit_claim", label: "Submit Claim" },
      { name: "action_expense_view_allowance", label: "View Allowance" },
    ],
    widgets: [
      { name: "widget_expense_history", label: "Expense History" },
      { name: "widget_expense_summary_cards", label: "Expense Summary Cards" },
    ],
  },
  {
    module: "module_projects",
    label: "Projects",
    fields: [
      { name: "field_project_name", label: "Project Name" },
      { name: "field_project_status", label: "Project Status" },
      { name: "field_project_budget", label: "Budget" },
      { name: "field_project_timeline", label: "Timeline" },
      { name: "field_project_members", label: "Members" },
    ],
    actions: [
      { name: "action_project_create", label: "Create Project" },
      { name: "action_project_manage_tasks", label: "Manage Tasks" },
      { name: "action_project_log_time", label: "Log Time" },
    ],
    widgets: [
      { name: "widget_project_list", label: "Project List" },
      { name: "widget_project_kanban", label: "Kanban Board" },
      { name: "widget_project_timesheet", label: "Timesheet" },
    ],
  },
  {
    module: "module_gps_tracking",
    label: "GPS Tracking",
    fields: [
      { name: "field_gps_latitude", label: "Latitude" },
      { name: "field_gps_longitude", label: "Longitude" },
      { name: "field_gps_speed", label: "Speed" },
      { name: "field_gps_accuracy", label: "Accuracy" },
    ],
    actions: [
      { name: "action_gps_view_live", label: "View Live Location" },
      { name: "action_gps_playback", label: "Journey Playback" },
    ],
    widgets: [
      { name: "widget_gps_live_map", label: "Live Map" },
      { name: "widget_gps_journey_history", label: "Journey History" },
    ],
  },
  {
    module: "module_retailers",
    label: "Retailers",
    fields: [
      { name: "field_retailer_name", label: "Retailer Name" },
      { name: "field_retailer_address", label: "Address" },
      { name: "field_retailer_contact", label: "Contact" },
      { name: "field_retailer_type", label: "Type" },
    ],
    actions: [
      { name: "action_retailer_add", label: "Add Retailer" },
      { name: "action_retailer_edit", label: "Edit Retailer" },
    ],
    widgets: [
      { name: "widget_retailer_list", label: "Retailer List" },
      { name: "widget_retailer_map", label: "Retailer Map" },
    ],
  },
  {
    module: "module_orders",
    label: "Orders",
    fields: [
      { name: "field_order_items", label: "Order Items" },
      { name: "field_order_total", label: "Total Amount" },
      { name: "field_order_retailer", label: "Retailer" },
      { name: "field_order_status", label: "Status" },
    ],
    actions: [
      { name: "action_order_create", label: "Create Order" },
      { name: "action_order_edit", label: "Edit Order" },
    ],
    widgets: [
      { name: "widget_order_list", label: "Order List" },
      { name: "widget_order_summary", label: "Order Summary" },
    ],
  },
  {
    module: "module_leaves",
    label: "Leaves",
    fields: [
      { name: "field_leave_type", label: "Leave Type" },
      { name: "field_leave_dates", label: "Leave Dates" },
      { name: "field_leave_reason", label: "Reason" },
      { name: "field_leave_balance", label: "Leave Balance" },
      { name: "field_leave_status", label: "Status" },
    ],
    actions: [
      { name: "action_leave_apply", label: "Apply Leave" },
      { name: "action_leave_cancel", label: "Cancel Leave" },
    ],
    widgets: [
      { name: "widget_leave_balance_cards", label: "Leave Balance Cards" },
      { name: "widget_leave_application_list", label: "Leave Applications List" },
    ],
  },
];

// Helper functions
export function getModuleByName(moduleName: string): HierarchicalModule | undefined {
  return HIERARCHICAL_MODULES.find((m) => m.module === moduleName);
}

export function getAllFieldNames(): string[] {
  return HIERARCHICAL_MODULES.flatMap((m) => m.fields.map((f) => f.name));
}

export function getAllActionNames(): string[] {
  return HIERARCHICAL_MODULES.flatMap((m) => m.actions.map((a) => a.name));
}

export function getAllWidgetNames(): string[] {
  return HIERARCHICAL_MODULES.flatMap((m) => m.widgets.map((w) => w.name));
}

export function getPermissionType(itemName: string): "module" | "field" | "action" | "widget" {
  if (itemName.startsWith("module_")) return "module";
  if (itemName.startsWith("field_")) return "field";
  if (itemName.startsWith("action_")) return "action";
  if (itemName.startsWith("widget_")) return "widget";
  return "module";
}

export function getParentModule(itemName: string): string | null {
  for (const mod of HIERARCHICAL_MODULES) {
    if (mod.fields.some((f) => f.name === itemName)) return mod.module;
    if (mod.actions.some((a) => a.name === itemName)) return mod.module;
    if (mod.widgets.some((w) => w.name === itemName)) return mod.module;
  }
  return null;
}

export function getItemLabel(itemName: string): string {
  for (const mod of HIERARCHICAL_MODULES) {
    if (mod.module === itemName) return mod.label;
    const field = mod.fields.find((f) => f.name === itemName);
    if (field) return field.label;
    const action = mod.actions.find((a) => a.name === itemName);
    if (action) return action.label;
    const widget = mod.widgets.find((w) => w.name === itemName);
    if (widget) return widget.label;
  }
  return itemName;
}
